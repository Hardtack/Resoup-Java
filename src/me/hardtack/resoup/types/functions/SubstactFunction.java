...
public class SubstactFunction extends BuiltinFunction {

	public SubstactFunction(Environment env) {
		super(env, new Str("-"));
	}
	...
	public Type call(List<Type> args) {
		if (args.size() < 2){
			...
			return new NumberT(Util.sub(0, n.getValue()));
		}
		NumberT result = new NumberT(((NumberT)args.get(0)).getValue());
		for(int i = 1; i < args.size(); i++){
			NumberT num  = (NumberT)args.get(i);
			result = new NumberT(Util.sub(result.getValue(), num.getValue()));
		}
		return result;
	}
}
